...
def get_content_by_pdf(file_path):
...
def get_single_word_list():
    list = set()
    try:
        with open('dataset/tu_viettat.txt', encoding="utf-8") as f:
            for line in f:
...
def get_ngram_prob(n):
    if n == 1:
        ...
        str = "trigram"
    with open('Vi_Spell_Check/dataset/' + str + '.txt', encoding="utf-8") as f:
        data = json.load(f)
    ngram = dict()
    total = data["_total"]
    for key in data:
        ngram[key] = data[key] / total;
    return ngram
